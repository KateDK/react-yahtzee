...
class SmallStraight extends Rule {
  evalRoll = (dice) => {
    ...
    console.log(d);
    // small straight must be 4 different dice & 1,2,3,4 OR 2,3,4,5 OR 3,4,5,6
    if (d.has(2) && d.has(3) && d.has(4)) {
      if (d.has(1) || d.has(5)) return this.score;
    }
    if (d.has(3) && d.has(4) && d.has(5)) {
      if (d.has(2) || d.has(6)) return this.score;
    }
    return 0;
  };
}
